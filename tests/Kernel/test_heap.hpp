...
void test_bump_raw()
{
    ...
    for (uint32_t n = 1; n < 4; n++) {
        void* ptr = kernel::heap::bump.allocate(n);
        ...
        runtime::text_output.put_string("\nused: ");
        runtime::text_output.put_uint(kernel::heap::bump.used());

        runtime::text_output.put_char('\n');
    }

    runtime::text_output.put_string("\nremaining: ");
    runtime::text_output.put_uint(reinterpret_cast<uint32_t>(kernel::heap::bump.remaining()));
...
void test_bump_mark()
{
    ...
    runtime::text_output.put_char('\n');
    const uintptr_t marker = kernel::heap::bump.mark();
    for (uint32_t n = 1; n < 4; n++) {
        void* ptr = kernel::heap::bump.allocate(n);
        uintptr_t addr = reinterpret_cast<uintptr_t>(ptr);
    ...
    kernel::heap::bump.rewind(marker);
